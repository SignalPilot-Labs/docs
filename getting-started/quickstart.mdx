...
<Check>
Ensure you have SignalPilot installed. If not, run:
```bash
uvx signalpilot
```
</Check>
...
<Steps>
<Step title="Launch SignalPilot">
  ```bash
  uvx signalpilot
  ```

  Your browser will open Jupyter Lab at `http://localhost:8888`

  <Check>
  **Expected:** SignalPilot panel visible in left sidebar
  </Check>
</Step>
...
<Step title="Load sample data">
  In the first cell, load a sample dataset:
  ...
  <Check>
  **Expected:** DataFrame with date and revenue columns displayed
  </Check>
</Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Investigate Revenue Drops" icon="chart-line-down" href="/guides/investigating-revenue-drops">
    Follow a real-world investigation guide
  </Card>
